...
{
    public class StartUserSessionTests : DatabaseHelper
    {
        [Fact]
        public async Task ValidationChecksInputs()
        {
            ...
            Assert.Equal(new[] { "User name is required", "Password is required" }, FakeEngine.GetErrors(errors));
        }
        ...
        public async Task ValidatePassesChecks()
        {
            ...
            using var store = InitialiseDatabase(new User { Id = "Tahi", Role = UserRole.Student, Name = "Bob", Password = Password.New("1234") });
