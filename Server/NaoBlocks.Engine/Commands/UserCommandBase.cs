...
namespace NaoBlocks.Engine.Commands
{
        ...
        : CommandBase
    {
        ...
        protected  async Task<User?> ValidateAndRetrieveUser(IDatabaseSession session, string? name, UserRole? role, List<CommandError> errors)
        {
            ...
            if (!errors.Any())
            {
                person = await session.Query<User>()
